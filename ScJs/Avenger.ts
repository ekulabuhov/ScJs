...
class Avenger extends createjs.Sprite {
    constructor() {
        var imgAvengerSS = <HTMLImageElement>ResourceLoader.get("unit/zerg/avenger.png");
    ...
    moveTo(x: number, y: number) {
        ...
        var distance: number = Math.sqrt(Math.pow(x - this.x, 2) + Math.pow(y - this.y, 2));
        ...
        var turn: number = Math.round(angle / 22.5);
